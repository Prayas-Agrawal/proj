...
class TevatronTrainer_deepquant(Trainer):
    def __init__(self, *args, **kwargs):
        super(TevatronTrainer_deepquant, self).__init__(*args, **kwargs)
        # self.tokenizer = self.okenizer
        self._dist_loss_scale_factor = dist.get_world_size() if self.args.negatives_x_device else 1
    ...
    def compute_loss(self, model, inputs):
        query, passage = inputs
        # print("query", query)
        # print("len", len(query), len(passage), len(passage["input_ids"]))
        # print("len nums", len(query_nums), len(query_nums["numbers"]))
        # print('asdfpsg', self.tokenizer.convert_ids_to_tokens(passage["input_ids"][1]), passage_nums["numbers"][1])
        # raise "ASfsd"

        return model(query=query, passage=passage).loss

    def training_step(self, *args):
        return super(TevatronTrainer_deepquant, self).training_step(*args) / self._dist_loss_scale_factor
    
    def log(self, logs: dict):
        super().log(logs)  # Call the original logging method
        if self.args.local_rank in [-1, 0]:  # Only log for the main process
            print(logs)  # Print ensures the log is immediately visible
